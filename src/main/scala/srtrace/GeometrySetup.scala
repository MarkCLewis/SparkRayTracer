...
object GeometrySetup {
	...
	*/
	...
	def readParticles(): Geometry = {
		...
		val carURL6 = new URL("http://www.cs.trinity.edu/~mlewis/Rings/AMNS-Moonlets/Moonlet4/CartAndRad.6029.bin")
		val carURL7 = new URL("http://www.cs.trinity.edu/~mlewis/Rings/AMNS-Moonlets/Moonlet4/CartAndRad.6028.bin")

  		val particles1 = CartAndRad.readStream(carURL.openStream).map(p => GeomSphere(Point(-3*2.0e-5-p.x, p.y, p.z), p.rad, _ => new RTColor(1, 1, 1, 1), _ => 0.0))
		val particles2 = CartAndRad.readStream(carURL2.openStream).map(p => GeomSphere(Point(-2*2.0e-5-p.x, p.y, p.z), p.rad, _ => new RTColor(1, 1, 1, 1), _ => 0.0))
		...
		val particles4 = CartAndRad.readStream(carURL4.openStream).map(p => GeomSphere(Point(p.x, p.y, p.z), p.rad, _ => new RTColor(1, 1, 1, 1), _ => 0.0))
		val particles5 = CartAndRad.readStream(carURL5.openStream).map(p => GeomSphere(Point(2.0e-5+p.x, p.y, p.z), p.rad, _ => new RTColor(1, 1, 1, 1), _ => 0.0))
		val particles6 = CartAndRad.readStream(carURL6.openStream).map(p => GeomSphere(Point(2*2.0e-5+p.x, p.y, p.z), p.rad, _ => new RTColor(1, 1, 1, 1), _ => 0.0))
		val particles7 = CartAndRad.readStream(carURL7.openStream).map(p => GeomSphere(Point(3*2.0e-5+p.x, p.y, p.z), p.rad, _ => new RTColor(1, 1, 1, 1), _ => 0.0))
