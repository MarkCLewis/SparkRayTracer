...
object EricasTestingFile {
  ...
  def giveOffsets(r: RDD[(Int, Int)], offsets: Array[(Double, Double)]) : RDD[(Int,(Int, Double, Double))] = {
  ...
  def main(args: Array[String]) = {
    ...
    val geom = createKDTrees(giveOffsets(divisionOfFiles(sc, numPartitions, cartAndRadNumbers), offsets))//.collect()//sc.parallelize(GeometrySetup.randomGeometryActualArr(new scala.util.Random(System.currentTimeMillis), maxX, minX,20,10,10,-10,2, 5)) //actual geometries
    ...
    println(giveOffsets(divisionOfFiles(sc, numPartitions, cartAndRadNumbers), offsets).collect().toList)


    //Map to RDD[(Int, KDTreeGeometry)]

    println(createKDTrees(giveOffsets(divisionOfFiles(sc, numPartitions, cartAndRadNumbers), offsets)).count())
